...
export default function Billing() {
  const { data: prices, isLoading, error } = useGetV1BillingPricesQuery(undefined);
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Buy credits</h2>
          <p className="text-red-600">Error loading prices. Please try again later.</p>
        </div>
      </div>
    );
  }
  ...
  if (!prices || prices.length === 0) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">Buy credits</h2>
          <p className="text-neutral-600">No pricing options available at the moment.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold tracking-tight">Buy credits</h2>
        <p className="text-neutral-600">Purchase credit packs for image generation.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {prices.map((price) => (
          <div
            ...
            className={`card cursor-pointer border-2 p-6 transition-colors ${
          ...
          >
            <div className="space-y-3">
              <div>
                <h3 className="text-lg font-semibold">{price.name ?? 'Untitled'}</h3>
                <p className="text-sm text-neutral-600">{price.credits ?? 0} credits</p>
              </div>
      ...
      {selectedPriceId && (
        <div className="card border-blue-200 bg-blue-50 p-4">
          <p className="text-sm text-blue-800">
            Selected: {prices.find((p) => (p.id ?? '') === selectedPriceId)?.name ?? 'Untitled'}(
            {prices.find((p) => (p.id ?? '') === selectedPriceId)?.credits ?? 0} credits)
          </p>
        </div>
      )}
    </div>
