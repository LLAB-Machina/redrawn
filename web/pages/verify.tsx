...
export default function Verify() {
  ...
  return (
    <div className="mx-auto max-w-md space-y-4">
      ...
      <p className="text-sm text-neutral-600">Please wait while we verify your magic link.</p>
      {isLoading && <div className="h-2 w-full animate-pulse rounded bg-neutral-200" />}
      {error && (
        <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-800">
          {error} <a className="ml-1 underline decoration-neutral-300 underline-offset-4 hover:text-red-900" href="/signup">Request a new link</a>.
        </div>
      )}
    </div>
  )
