...
export default function PublicAlbum() {
  ...
  const { data: album } = useGetV1PublicAlbumsBySlugQuery(key, { skip: !key })
    ...
    <div className="space-y-6">
      ...
      {(album?.photos && album.photos.length > 0) ? (
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {album.photos.map((p) => (
            <PublicAlbumImage key={p.id} fileId={p.file_id || undefined} ensureUrl={ensureFileUrl} />
          ))}
        </div>
      ) : (
...
function PublicAlbumImage({ fileId, ensureUrl }: { fileId?: string; ensureUrl: (id?: string | null) => Promise<string | null> }) {
  const [src, setSrc] = useState<string | null>(null)
  useEffect(() => { ensureUrl(fileId).then(setSrc) }, [fileId])
  return (
    <div className="aspect-[4/3] overflow-hidden rounded-lg bg-neutral-200 ring-1 ring-inset ring-neutral-300">
      {src ? <img src={src} alt="photo" className="h-full w-full object-cover" /> : null}
    </div>
  )
}
