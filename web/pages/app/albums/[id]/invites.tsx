...
const ROLE_OPTIONS: SelectOption[] = [
  { value: "viewer", label: "Viewer", description: "Can view photos and albums" },
  { value: "editor", label: "Editor", description: "Can add photos and edit content" },
  { value: "owner", label: "Owner", description: "Full access including deleting album" },
];
...
export default function AlbumInvites() {
  ...
  const { data: memberships, refetch } = useGetV1AlbumsByIdMembershipsQuery(id ? { id } : (undefined as any), { skip: !id });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <p className="text-xs text-neutral-600 mt-1">Share this link for public viewing.</p>
        <div className="mt-2 flex gap-2 items-center">
          <input className="input h-9 flex-1" readOnly value={publicUrl} />
          <button className="btn btn-neutral h-9" onClick={() => navigator.clipboard.writeText(publicUrl)} type="button">Copy</button>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        >
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-neutral-600 mt-1">Send an invitation to join this album</p>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-neutral-700 mb-1">Email Address</label>
                <input className="input h-9 w-full" name="email" placeholder="person@example.com" type="email" required />
              </div>
              <div>
                <label className="block text-xs font-medium text-neutral-700 mb-1">Role</label>
                <Select name="role" options={ROLE_OPTIONS} defaultValue="viewer" className="h-9 w-full" />
              </div>
            </div>
            <button className="btn btn-primary h-9 w-full" type="submit">Send Invite</button>
          </div>
        </form>
        ...
        <form
          ...
          onSubmit={async (e: any) => {
            ...
            const role = (e.currentTarget.elements.namedItem("role") as HTMLInputElement).value;
            try {
              await createLink({ id, createInviteLinkRequest: { role } }).unwrap();
              await refetch();
            } catch (err: any) {
              alert(String(err));
            }
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-xs text-neutral-600 mt-1">Generates a link that grants a role to anyone who signs in</p>
            <Select name="role" options={ROLE_OPTIONS} defaultValue="viewer" className="h-9 w-full" />
            <button className="btn btn-neutral h-9 w-full" type="submit">Create Link</button>
          </div>
        </form>
      </div>
            ...
            memberships.links.map((l) => {
              ...
              return (
                <div key={l.id} className="flex items-center gap-2">
                  ...
                  <button className="btn btn-neutral h-9" onClick={() => navigator.clipboard.writeText(joinUrl)} type="button">Copy</button>
                  <button
        ...
        <div className="mt-2 space-y-2">
          {memberships?.members?.map((m) => (
            <div key={m.user_id} className="flex items-center justify-between gap-3">
              <div className="text-sm">{m.email} — <span className="text-neutral-600">{m.role}</span></div>
              <button className="btn btn-danger h-8" onClick={async () => { if (!m.user_id) return; await removeMember({ id, userId: m.user_id }).unwrap(); await refetch(); }}>Remove</button>
            </div>
          ...
          {memberships?.invites?.length ? (
            memberships.invites.map((i) => (
              <div key={i.id} className="text-sm">
                {i.email} — <span className="text-neutral-600">{i.role}</span> <span className="ml-1 text-xs">({i.status})</span>
              </div>
            ))
          ) : (
      ...
      <div className="grid max-w-md gap-6">
        <form
          ...
          onSubmit={async (e: any) => {
            ...
            const role = (e.currentTarget.elements.namedItem("role") as HTMLInputElement).value;
            try {
              const data = await setRole({ id, userId, roleRequest: { role } }).unwrap();
        ...
        >
          ...
          <button className="btn btn-neutral h-9" type="submit">Set Role</button>
        </form>
        ...
        >
          ...
          <input className="input h-9" name="user_id" placeholder="user id" />
          <button className="btn btn-danger h-9" type="submit">Remove</button>
        </form>
      </div>
    </div>
  );
