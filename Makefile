...
openapi: ## Generate OpenAPI JSON into api/openapi.json without running server (deterministic order, formatted)
	cd api && go run ./cmd/api --openapi-only | jq -S . > openapi.json
	cd web && npx prettier --write ../api/openapi.json
