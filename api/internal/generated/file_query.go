...
type FileQuery struct {
	...
	withOriginalOf  *OriginalPhotoQuery
	withGeneratedOf *GeneratedPhotoQuery
...
// QueryOriginalOf chains the current query on the "original_of" edge.
func (_q *FileQuery) QueryOriginalOf() *OriginalPhotoQuery {
...
// QueryGeneratedOf chains the current query on the "generated_of" edge.
func (_q *FileQuery) QueryGeneratedOf() *GeneratedPhotoQuery {
...
func (_q *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withOriginalOf:  _q.withOriginalOf.Clone(),
		withGeneratedOf: _q.withGeneratedOf.Clone(),
...
// WithOriginalOf tells the query-builder to eager-load the nodes that are connected to
// the "original_of" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *FileQuery) WithOriginalOf(opts ...func(*OriginalPhotoQuery)) *FileQuery {
	query := (&OriginalPhotoClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOriginalOf = query
	return _q
}

// WithGeneratedOf tells the query-builder to eager-load the nodes that are connected to
// the "generated_of" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *FileQuery) WithGeneratedOf(opts ...func(*GeneratedPhotoQuery)) *FileQuery {
	query := (&GeneratedPhotoClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGeneratedOf = query
	return _q
}
...
func (_q *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withOriginalOf != nil,
			_q.withGeneratedOf != nil,
		}
	)
	...
	if query := _q.withOriginalOf; query != nil {
		if err := _q.loadOriginalOf(ctx, query, nodes,
			func(n *File) { n.Edges.OriginalOf = []*OriginalPhoto{} },
			func(n *File, e *OriginalPhoto) { n.Edges.OriginalOf = append(n.Edges.OriginalOf, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGeneratedOf; query != nil {
		if err := _q.loadGeneratedOf(ctx, query, nodes,
			func(n *File) { n.Edges.GeneratedOf = []*GeneratedPhoto{} },
			func(n *File, e *GeneratedPhoto) { n.Edges.GeneratedOf = append(n.Edges.GeneratedOf, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *FileQuery) loadOriginalOf(ctx context.Context, query *OriginalPhotoQuery, nodes []*File, init func(*File), assign func(*File, *OriginalPhoto)) error {
	...
	query.Where(predicate.OriginalPhoto(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.OriginalOfColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.file_original_of
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_original_of" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_original_of" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *FileQuery) loadGeneratedOf(ctx context.Context, query *GeneratedPhotoQuery, nodes []*File, init func(*File), assign func(*File, *GeneratedPhoto)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*File)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.GeneratedPhoto(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.GeneratedOfColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.file_generated_of
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_generated_of" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_generated_of" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
